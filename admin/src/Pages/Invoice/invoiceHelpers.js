// admin/src/Pages/Invoice/invoiceHelpers.js
...
export function numberToWords(amount, currencyWord = "Taka") {
  ...
  return `${parts.join(" ")} ${currencyWord} Only`.replace(/\s+/g, " ").trim();
}

export function sanitizeText(value, maxLen = 500) {
  if (typeof value !== "string") return "";
  return value.replace(/<[^>]*>?/gm, "").replace(/javascript:/gi, "").slice(0, maxLen);
}
